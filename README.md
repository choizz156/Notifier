...
- [6. 비동기 처리 구조](#6-비동기-처리-구조)
- [7. 미구현 / 제약사항/개선할 점](#7-미구현--제약사항개선할-점)
...
* `공통 알림`은 상대적으로 중요도가 낮다고 가정하고 설계했습니다.
...
* 대량 알림 발송 시 발생할 수 있는 데이터 팽창 및 메모리 과부하 위험이 존재했습니다.
...
대량 알림 발송 시 발생할 수 있는 데이터 팽창 및 메모리 과부하 위험이 존재했기 때문에
`공통 알림`과 `개인 알림`을 분리하여 설계했습니다.
...
* **공통 알림**
  ...
  * **안전망 (DLQ):**
    * 이 대량 발행 및 분배 과정 중 서버 에러가 나면, 해당 요청 원본은 `public_notification_dlq` 테이블에 저장되어 스케줄러를 통해 나중에 복구됩니다.
      ![public](./images/public.png)
* **공통 로직**
  개인 알림이 쏜 1건의 이벤트든, 공통 알림이 쪼개서 쏜 수만 건의 이벤트든 **메시지 버스에 도달한 직후부터는 둘의 구분이 사라집니다.** 완전히 동일한 파이프라인을 탑니다.
  * **수신 및 격리:**
...
* **로그 데이터(NotificationLogEntity)**:
  * 이벤트 발행 및 발송 처리 과정에서 중복 로그가 적재되는 것을 막기 위해 `[reference_id, reference_type, channel_type, subscriber_id]` 같은 식별 데이터뿐만 아니라 **이벤트 상태(`event_status`)와 재시도 카운트(`retry_count`)까지** 유니크 인덱스에 포함시켜, 동일 재시도 단계에서의 중복 INSERT를 차단하게 했습니다.
...
### (7) 진행 중인 프로세스가 장기 진행 중 상태일 경우의 복구
...
* 5분 주기로 스케쥴링을 설정한 이유
  * 재시도 전략 중 **STANDARD와 MINIMUM 정책은 모든 재시도가 최대 3분 이내에 완전히 종료**되도록 설계했습니다. 즉, 3분이 지난 후에도 상태가 `PROCESSING`에 머물러 있다면 이는 명백한 오류입니다.
...
* 알림의 중요도에 따라 재시도 전략(Aggressive, Standard, Minimum)이 다르기 때문에, 복구 대상을 스캔할 때 일괄적인 재시도 시간을 적용하지 않았습니다.
...
* 다중 서버 환경에서 여러 인스턴스가 동시에 재시작될 경우, 모든 인스턴스가 미처리 데이터를 스캔하여 동일한 알림을 여러 번 중복 발송하는 상황이 발생할 수 있습니다.
...
시스템 장애나 일시적인 네트워크 오류 등으로 인해 알림 발송이 명시적으로 실패하거나 처리 도중 중단될 수 있습니다. 이러한 유실을 방지하기 위해, 수동 재시도 API가 단순 실패(`FAIL`) 건뿐만 아니라 처리 도중 멈춰버린 미처리(`PENDING`) 건들까지 포괄하여 재발행(`retry`)하도록 구성했습니다.
...
대량 발송 성격이 짙은 **공통 알림 기능에 한정하여 예약 발송을 적용**했습니다. 발송 예약 시간은 매 시간 **정각 단위**로 설정하도록 설계했습니다.

**특정 시점 발송 파이프라인 분리**:

* 예약 알림 전용 데이터(`ReservationNotificationEntity`)를 별도로 분리하고, `ReservationNotificationScheduler`가 주기적으로 현재 시간과 비교해 발송 시점이 도달한 예약 건만 발행하도록 설계했습니다.
...
* admin 모듈에서 수행합니다.
* 소스 코드 수정 없이 백오피스 환경에서 운영자가 직접 템플릿 내용을 관리할 수 있도록 관리의 유연성을 확보했습니다.
...
* 예외를 던지고 발송을 포기하는 대신 내장 템플릿으로 안전하게 우회하도록 처리함으로써, 외부 장애 요인에도 알림 발송이 실패하지 않도록 가용성을 확보했습니다.
...
**대용량 데이터 환경에서의 성능 및 확장성(N+1 방지):**

* 알림 시스템 특성상 하나의 엔티티(예: 유저, 공통 알림 등)에 수천, 수만 건의 발송 이력(Log)이 쌓일 수 있습니다.
* 이를 `@OneToMany`로 매핑할 경우, 영속성 컨텍스트가 불필요하게 대량의 컬렉션을 관리하게 되어 심각한 메모리 낭비와 쿼리 성능 저하를 유발할 수 있습니다.

**도메인 결합도 감소 및 Aggregate 분리**:

* 발송 대상 정보(Notification)와 발송 이력(NotificationLog)은 라이프사이클과 변경 주기가 다릅니다.이들을 강한 외래키(FK)나 객체 참조로 묶어두는 대신 `referenceId` 값만 가지도록 느슨하게 결합하여 도메인 간의 결합도를 낮췄습니다.

**아키텍처 확장 가능성**

* 객체 참조를 끊어냄으로써 향후 무한히 증가하는 알림 로그(Log) 데이터를 별도의 로그 전용 DB(NoSQL 등)나 외부 마이크로서비스로 분리하기 쉬운 유연한 아키텍처를 설계했습니다.
